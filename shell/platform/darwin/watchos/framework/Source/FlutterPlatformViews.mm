...
void FlutterPlatformViewsController::SetFlutterView(WKInterfaceObject* flutter_view) {
  flutter_view_.reset([flutter_view retain]);
}

void FlutterPlatformViewsController::SetFlutterViewController(
    WKInterfaceController* flutter_view_controller) {
...
void FlutterPlatformViewsController::OnCreate(FlutterMethodCall* call, FlutterResult& result) {
  ...
  root_views_[viewId] = fml::scoped_nsobject<WKInterfaceObject>([touch_interceptor retain]);
...
WKInterfaceObject* FlutterPlatformViewsController::ReconstructClipViewsChain(int number_of_clips,
                                                                  WKInterfaceObject* platform_view,
                                                                  WKInterfaceObject* head_clip_view) {
  ...
  WKInterfaceObject* head = platform_view;
  int clipIndex = 0;
...
void FlutterPlatformViewsController::CompositeWithParams(int view_id,
  ...
  WKInterfaceObject* touchInterceptor = touch_interceptors_[view_id].get();
  ...
  if (currentClippingCount != previousClippingCount) {
    ...
    WKInterfaceObject* oldPlatformViewRoot = root_views_[view_id].get();
    WKInterfaceObject* newPlatformViewRoot =
        ReconstructClipViewsChain(currentClippingCount, touchInterceptor, oldPlatformViewRoot);
    root_views_[view_id] = fml::scoped_nsobject<WKInterfaceObject>([newPlatformViewRoot retain]);
  }
...
void FlutterPlatformViewsController::Reset() {
  WKInterfaceObject* flutter_view = flutter_view_.get();
  for (WKInterfaceObject* sub_view in [flutter_view subviews]) {
...
bool FlutterPlatformViewsController::SubmitFrame(GrContext* gr_context,
  ...
  for (size_t i = 0; i < composition_order_.size(); i++) {
    ...
    // `FlutterView`.
    WKInterfaceObject* platform_view_root = root_views_[view_id].get();
    WKInterfaceObject* overlay = overlays_[view_id]->overlay_view;
...
void FlutterPlatformViewsController::DisposeViews() {
  ...
  for (int64_t viewId : views_to_dispose_) {
    WKInterfaceObject* root_view = root_views_[viewId].get();
...
- (instancetype)initWithTarget:(id)target
         flutterViewController:(WKInterfaceController*)flutterViewController;
@end

@implementation FlutterTouchInterceptingView {
  fml::scoped_nsobject<DelayingGestureRecognizer> _delayingRecognizer;
}
- (instancetype)initWithEmbeddedView:(WKInterfaceObject*)embeddedView
               flutterViewController:(WKInterfaceController*)flutterViewController {
...
@implementation ForwardingGestureRecognizer {
  ...
  // will go away.
  WKInterfaceController* _flutterViewController;
